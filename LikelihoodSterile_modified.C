...
void fcn(Int_t& npar, Double_t* gin, Double_t& f, Double_t *par, Int_t iflag){
 ...
 temp=y*(log(insidelog))-insidelog;
  if ( y>0&& pdf>0 &&energy[0]>Min,energy[0]<Max){
...
void  LikelihoodSterile_modified(){
const int npar=4;
  //TH1::SetDefaultSumw2();
...
for (int i=0; i<npar; i++){
 ...
 gMinuit->Print();
   ...
   */
 cout<<"integral of the fake data--------------------------------------"<<hfakedatabetadecay->Integral(hfakedatabetadecay->FindBin(0.55),hfakedatabetadecay->FindBin(0.58))<<endl;
 cout<<"--------------------------------------------"<<endl;
