...
void fcn(Int_t& npar, Double_t* gin, Double_t& f, Double_t *par, Int_t iflag){
  ...
  if ( y>0&& pdf>0 &&energy[0]<Max &&energy[0]>Min){
    ...
    temp=y*(log(insidelog))-insidelog-(((par[1]-0.565)*(par[1]-0.565))/2/0.002/0.002);
...
void  LikelihoodSterile_modified5(int ifile){
  //output histograms
  TH1F *hq=new TH1F("hq","Q par",50,0.56,0.57);
  TH1F *hqer=new TH1F("hqer","Q parerror",50,1.0E-11,1.0E-4);

  TH1F *hsin2theta=new TH1F("hsin2theta","sin2theta",25,1.0E-5,1.0E-3);
  TH1F *hsin2thetaerror=new TH1F("hsin2thetaerror","sin2theta error",25,1.0E-5,1.0E-3);
...
ifstream spectrum_file;
 ...
 TH1F *hpileuppar=new TH1F("hpileuppar","pile up par",200,0.5,1.5);
  ...
  TH1F *hqerror=new TH1F("hqerror","Q Error",50,1.0E-6,0.06E-3);
  ...
  TFile *fout=new TFile(Form("/home/fatemigh/projects/rpp-jillings/fatemigh/sterile/Likelihood/fitresultsrootpileup/fitresultswithpileup%i.root",ifile),"recreate");
...
for (int i=0; i<npar; i++){
 ...
 for (int ipar=0;ipar<npar;ipar++){
  ...
  hsin2theta->Fill(currentValue[2]);
  hsin2thetaerror->Fill(currentError[2]);
